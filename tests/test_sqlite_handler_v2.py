...
class TestSQLiteHandler(unittest.TestCase):
    ...
    def test_db_alter_column(self):
        self.db.alter_column(table_name=self.table_name, column_name='user', new_column_name='username')
        current_columns = self.db.get_columns(table_name=self.table_name)
        self.assertTrue('username' in current_columns and 'user' not in current_columns)
    ...
    def test_db_update_rows(self):
        ...
        rowcount = self.db.update_rows(
            table_name=self.table_name,
            set_clauses={'Register_datetime': datetime(2020, 2, 1)},
            where_clauses=where_clauses
        )
